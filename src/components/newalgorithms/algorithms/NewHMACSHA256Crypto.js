...
const NewHMACSHA256Crypto = () => {
  ...
  const handleHmac = async () => {
      ...
      try {
        const response = await HMACSHA256(key, message,outputFormat);
  ...
  return (
    <Box sx={{ width: '100%' }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'flex-start' }}>
          ...
          <Box>
            <Typography variant="body1" sx={{ mb: 1, fontWeight: 500, color: '#424242' }}>
              HMAC-SHA256是一种基于SHA256哈希函数的消息认证码，用于验证数据完整性和来源真实性。
            </Typography>
            <Typography variant="body2" color="text.secondary">
              HMAC-SHA256常用于API认证、数据完整性校验、安全通信等场景，通过使用密钥将其与消息组合后进行哈希运算，确保消息的完整性和真实性。相比HMAC-SHA1，它提供了更强的安全性。
            </Typography>
          </Box>
        </Box>
      </Paper>
      ...
      <Paper elevation={1} sx={{ p: 3, mb: 3, bgcolor: 'white' }}>
        ...
        >
          {loading ? 'HMAC计算中...' : '计算HMAC-SHA256值'}
        </Button>
